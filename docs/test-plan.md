...
**Ambiente:** um projeto Supabase dedicado a testes. Nao usar o banco de desenvolvimento — ver
...
**Total: 16 User Stories, 10 casos de integracao, 13 servicos candidatos a teste de unidade.**
...
| Item | Definicao |
| --- | --- |
| Suporte | PostgreSQL 16 no Supabase |
| Autenticacao | Supabase Auth |
| Sessao | Tokens do Supabase, persistidos no navegador |
| Isolamento | RLS por `auth.uid()` |
| Ambiente | Projeto Supabase dedicado, separado do de desenvolvimento |
...
| --- | --- | --- |
| **RLS so e criado se `auth.users` existir** | Alto. No PostgreSQL local o RLS nunca e aplicado, entao o ambiente de teste pode passar vazando dados sem acusar erro. | Testar contra um projeto Supabase real, nunca so com Postgres puro. Ver IT-01 e IT-02. |
...
| **Babel sem configuracao** | Alto. `babel-jest` nao converte ESM e a suite nao carrega. | `babel.config.js` com `preset-env`. Issue #26. |
| **Ausencia de ambiente de teste dedicado** | Medio. Sem ele, IT-01 a IT-10 rodariam contra o banco de desenvolvimento. | Issue #20. |
...
| Testes de integracao | Jest + projeto Supabase de teste | IT-01 a IT-10 |
...
| Banco | PostgreSQL 16 (Supabase) | Persistencia e verificacao de constraints |

**Nota sobre ESLint:** o projeto nao usa ESLint, e a ausencia e uma decisao consciente — a base de
codigo nao compilaria sob ele sem migracao. O SonarQube cobre analise estatica sem exigir essa
ferramenta. Registrado aqui para que a decisao nao passe por esquecimento.
